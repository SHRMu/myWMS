...
public class StockStorageManageHandler {
    ...
    Map<String, Object> importStorageRecord(@RequestParam("file") MultipartFile file) throws StorageManageServiceException {
        ...
        if (file != null) {
            Map<String, Object> importInfo = stockStorageManageService.importStorage(file);
            if (importInfo != null) {
    ...
    public void exportStorageRecord(@RequestParam("searchType") String searchType, @RequestParam("keyword") String keyword,
        ...
        String fileName = "stockStorageRecord.xlsx";
...
//            repositoryBelong = sessionRepositoryBelong.toString();
        ...
        List<StockStorage> stockStorageList = null;
        Map<String, Object> queryResult = query(searchType, keyword, batchBelong, StatusUtil.DEFAULT_REPOSITORY_ID, -1, -1);
        if (queryResult != null)
            stockStorageList = (List<StockStorage>) queryResult.get("data");

        File file = stockStorageManageService.exportStorage(stockStorageList);
        if (file != null) {
            // 设置响应头

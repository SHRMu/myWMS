...
import de.demarks.wms.common.service.Interface.RepositoryService;
...
public class RepositoryManageServiceImpl implements RepositoryService {
    ...
    public boolean deleteRepository(Integer repositoryId) throws RepositoryManageServiceException {
        ...
        try {
            // 检查是否存在出库记录
            List<StockOutDO> stockOutDOList = stockOutMapper.selectAll(null,repositoryId);
            if (stockOutDOList != null && !stockOutDOList.isEmpty())
                return false;

            // 检查是否存在入库记录
            List<StockInDO> stockInDOList = stockInMapper.selectAll(null,repositoryId);

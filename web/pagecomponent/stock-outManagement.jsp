...
function fetchStorage(){
	$('#batch_selector').change(function(){
		stockout_batch = $(this).val();
		loadStorageInfo();
	});
}
//加载待出库良品库存
function loadStorageInfo(){
	if(stockout_batch != null && stockout_goods != null){
		$.ajax({
			type : 'GET',
			...
			data : {
				...
				batchBelong : stockout_batch,
				repositoryBelong : $('#repository_selector').val(),
				keyword : stockout_goods
			},
			success : function(response){
				if(response.total > 0){
					// data = response.rows[0].number;
					data = response.rows[0].passed;
